...
function find_user_rights($mainmenu, $user)
{
	...
	/* $inner_mainmenu = select_query_json("select VEWVALU from SRM_MENU mnu, SRM_MENU_ACCESS acc
												...
												order by VEWVALU DESC, mnu.MNUCODE Asc", "Centra", 'TEST'); */
	$access = 0;
...
function insert_req_dbquery($field_values, $tbl_names)
{
	try {
		...
		$save_query = save_query_json($sql_insert, "Req", 'TEST');
		return $save_query;
	}
	catch(Exception $e) {
		echo 'Message: ' .$e->getMessage();
...
function insert_test_dbquery($field_values, $tbl_names)
{
	...
	try {
		...
		$kvl = rtrim($kvl, ", ");
	   	...
	   	$sql_insert ="insert into ".$tbl_names." (".$kvl.") values (".$kyvl.")"; # DO NOT REMOVE ECHO FOR NEED RETURN USE insert_test1_dbquery function
...
function update_req_dbquery($field_values, $tbl_names, $where_condition)
{
	try{
		...
		$sql_update = "UPDATE ".$tbl_names." SET ".$kyvl." WHERE ".$where_condition;
		$save_query = save_query_json($sql_update, "Req", 'TEST');
		return $save_query;
	}
	catch(Exception $e) {
		//echo 'Message: ' .$e->getMessage();
...
function select_query_login_check_json($usrlogin, $usrpass, $brn_connection = 'Centra', $schema_source = 'TEST') {
	...
	$get_parameter->C_Mode = $schema_source; // 'TEST' / 'TEST';
...
function select_test_query_json($sqlqry_select, $brn_connection = 'Centra', $schema_source = 'TEST') {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
	...
	$get_parameter->B_Code = $brn_connection; // 'Centra';
	$get_parameter->C_Mode = $schema_source; // 'TEST';
	// $get_parameter->C_Mode='TEST';
	try{
...
function select_query_json($sqlqry_select, $brn_connection = 'Centra', $schema_source = 'TEST') {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
...
function select_query_json_new($sqlqry_select, $brn_connection = 'Centra', $schema_source = 'TEST') {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
...
function select_dbquery_json($sqlqry_select, $brn_connection = 'Centra', $schema_source = 'TEST') {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
	$opts = array(
        'http' => array(
            'user_agent' => 'PHPSoapClient'
...
function store_procedure_query_json_req($sqlqry_select, $brn_connection = 'Req', $schema_source = 'TEST') {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
...
function store_procedure_query_json1($sqlqry_select, $brn_connection = 'Centra', $schema_source = 'TEST') {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
...
function store_procedure_query_json2($sqlqry_select, $brn_connection = 'Centra', $schema_source = 'TEST') {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
...
function select_query_webservice($sqlqry_select) {
	// $service_url = 'http://tcstextile.in/tcs_service/TESTservice.asmx/App_TRN_List?TRN=A';
