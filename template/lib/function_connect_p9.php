...
function insert_dbquery_p9($field_values, $tbl_names)
{
	try {
		$key_value = array_keys($field_values);
		$org_value = array_values($field_values);
		...
		$save_query = save_query_json_p9($sql_insert);
		return $save_query;
	}
	catch(Exception $e) {
		echo 'Message: ' .$e->getMessage();
...
function select_query_json_p9($sqlqry_select, $brn_connection = 'Centra', $schema_source = 'TCS') {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
    ...
    $context = stream_context_create($opts);
    $wsdlUrl = "http://172.16.0.167:8050/cdata.asmx?Wsdl";
    $soapClientOptions = array(
        'stream_context' => $context,
        'exceptions'=> 1, 
        'trace' => 1,
	...
	$get_parameter->Cmdstr = $sqlqry_select;
	echo $sqlqry_select;
	try{
		$get_result=$client->GetDataset_Report($get_parameter)->GetDataset_ReportResult;
		print_r($get_result);
	}
	catch(SoapFault $fault){
		echo "<br><br>Fault code:{$fault->faultcode}";
...
// Save Query JSON Response
function save_query_json_p9($sqlqry_select) {
	// $client = new SoapClient("http://templive.thechennaisilks.com:5088/service.asmx?Wsdl", array('exceptions'=>true, 'trace' => true));
	$opts = array(
        'http' => array(
            'user_agent' => 'PHPSoapClient'
	...
	$get_parameter->str = $sqlqry_select;
	try{
		$get_result = $client->Store_Data_Report($get_parameter)->Store_Data_ReportResult;
	}
	catch(SoapFault $fault){
		$get_result = 0;
		/* echo "Fault code:{$fault->faultcode}".NEWLINE;
