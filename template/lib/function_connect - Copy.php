...
function find_user_rights($mainmenu, $user)
{
	...
	*/
	...
	$inner_mainmenu = select_query_json("select acc.mnucode from  SRM_MENU_ACCESS acc 
												where acc.vewvalu='Y' and  acc.entsrno = ".$user." 
												order by acc.mnucode Asc", "Centra", 'TCS');

	if(count($inner_mainmenu))
	{
		return 1; 
	}else{
		return 0;	
