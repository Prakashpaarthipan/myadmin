...
function add_ivuser1($md_user, $alternate_user, $txt_approval_number, $maxarqcode) {
	...
	if(count($sql_akiv) > 0) {
		...
		$maxarqcode1 = select_query_json("Select  nvl(Max(ARQSRNO),1)+1 maxarqsrno
												From APPROVAL_REQUEST WHERE APRNUMB='".$txt_approval_number."'", "Centra", 'TCS');

		$ivqry = delete_dbquery("INSERT INTO APPROVAL_request select ARQCODE,ARQYEAR,(".$maxarqcode1[0]['MAXARQSRNO']."),ATYCODE,ATMCODE,APMCODE,ATCCODE,APPRFOR,REQSTTO,APPRSUB,APPRDET,APPRSFR,APPRSTO,APPATTN,APRQVAL,APPDVAL,APPFVAL,BRNCODE,DEPCODE,TARNUMB,TARBALN,TARDESC,'".$fstusr[0]['EMPSRNO']."','".$fstusr[0]['EMPCODE']." - ".$fstusr[0]['EMPNAME']."','".$fstusr[0]['DESCODE']."','".$fstusr[0]['ESECODE']."','".$fstusrdesignation[0]['DESNAME']."','".$fstusrsection[0]['ESENAME']."','".$fstusr1[0]['EMPSRNO']."','".$fstusr1[0]['EMPCODE']." - ".$fstusr1[0]['EMPNAME']."','".$fstusr1[0]['DESCODE']."','".$fstusr1[0]['ESECODE']."','".$fstusrdesignation1[0]['DESNAME']."','".$fstusrsection1[0]['ESENAME']."',RQESTTO,RQTODES,RQTODSC,RQTOESC,RQTODSN,RQTOESN,APRNUMB,'N','I',APPINTP,10129,INTPDES,INTPDSC,INTPESC,INTPDSN,INTPESN,INTPAPR,INTSUGG,INTPFRD,INTPTOD,'".$fstusr[0]['EMPSRNO']."',ADDDATE,EDTUSER,EDTDATE,DELETED,'".$fstusr[0]['EMPSRNO']."',DELDATE,APRCODE,APRHURS,APRDAYS,APRDUED,'VERIFY THIS APPROVAL',APRTITL,FINSTAT,FINUSER,FINCMNT,FINDATE,TARVLCY,TARVLLY,EXPNAME,TARPRCY,TARPRLY,USRSYIP,PRJPRCS,PLANVAL,IMDUEDT,IMUSRCD,IMSTATS,IMFINDT,IMUSRIP,TYPMODE,SUBCORE,BUDTYPE,BUDCODE,IMFNIMG,NXLVLUS,PRICODE,SUPCODE,SUPNAME,SUPCONT,PRODWIS,RESPUSR,ALTRUSR,RELAPPR,ORGRECV,ORGRVUS,ORGRVDT,ORGRVDC,CNVRMOD,PURHEAD,APPTYPE,ADVAMNT,AGNSAPR,WRKINUSR,ARQPCOD,BDPLANR,DYNSUBJ,TXTSUBJ,RMQUOTS,RMBDAPR,RMCLRPT,RMARTWK,RMCONAR,WARQUAR,CRCLSTK,PAYPERC,FNTARDT,RPTUSER,ACKUSER,ACKSTAT,ACKDATE,AGEXPDT,AGADVAM,DYSBFDT,DYSBTDT,APPICON,RTFNUSR,RMPONUM,AUTFRWD,EXPSRNO,QUOYEAR,AREASON from APPROVAL_request where APRNUMB = '".$txt_approval_number."' and ARQSRNO=".$maxarqcode);
	}
	return $ivqry;
}
// Add internal Verification user dynamically FOR BID
...
function add_ivuser_product_test($md_user, $alternate_user, $txt_approval_number, $maxarqcode) {
	$ivqry = 0;
	$sql_akiv = select_query_json("select COUNT(*) CNT from APPROVAL_PRODUCTWISE_BUDGET where aprnumb = '".$txt_approval_number."' and PBDSRNO=".$maxarqcode." AND (REQSTFR = 21344 and REQSTBY <> 168)", "Centra", "TCS");
	if(count($sql_akiv) > 0) {
...
// If we send usercode, it will return the employee branch logo and text - 06-11-2017
function find_branch_logo($brncode) {
	$sql_empbrn = select_query_json("select brn.brncode, brn.brnname, brn.nicname, regexp_replace(SubStr(nicname,1,4),'[0-9]','')||SubStr(nicname,5,10) branch
											from branch brn
											where brn.BRNCODE in (".$brncode.")");
	switch ($sql_empbrn[0]['BRNCODE']) {
		case 201:
		case 202:
		case 205: // KTM
		...
		case 33: // Nachaas
			$brnlogo = "<img src='images/nachaas.jpg' border=0 style='max-width:160px; height:auto; vertical-align:middle;' align='middle'><span style='vertical-align:middle'> - ".$sql_empbrn[0]['BRANCH']."</span>";
			break;
	...
	for($int_i = 0; $int_i < count($userlisting); $int_i++) {
		...
		if($initial_srno == 1) {
			...
			if($sql_user[0]['EMPCODE'] == 3) {
				delete_dbquery("update APPROVAL_REQUEST set RQESTTO = '21344', RQTODES = '3 -  S KAARTHI', RQTODSC = '194', RQTOESC = '151', RQTODSN = 'DIRECTOR', RQTOESN = '001 OWNER' where aprnumb like '".$aprnumb."' and RQESTTO in (20118, 21344)");
			}
			// Only PS Madam
			if($sql_user[0]['EMPCODE'] == 2) {
				delete_dbquery("update APPROVAL_REQUEST set RQESTTO = '43400', RQTODES = '2 -  PADHMA SIVLINGAM S', RQTODSC = '194', RQTOESC = '151', RQTODSN = 'DIRECTOR', RQTOESN = '001 OWNER' where aprnumb like '".$aprnumb."' and RQESTTO in (21344, 20118)");
			}
			// Only KS Sir
			if($sql_user[0]['EMPCODE'] == 1) {
				delete_dbquery("update APPROVAL_REQUEST set RQESTTO = '20118', RQTODES = '1 -  SIVALINGAM K', RQTODSC = '193', RQTOESC = '151', RQTODSN = 'MANAGING DIRECTOR', RQTOESN = '001 OWNER' where aprnumb like '".$aprnumb."' and RQESTTO in (43400, 21344)");
			} else {
...
function generate_multiple_branch($txt_approval_number, $slt_topcore) {
	...
	foreach($sql_descode as $sectionrow) { $ai++;
		...
		// $apprno = "".$vall[0]." ".$srno." ".$valll[0]." ".substr($srno,-4)." ".$valll[1].""; RQTOESN

		$currentdate1 = strtoupper(date('d-m-Y'));
		$currenttime1 = strtoupper(date('h:i A'));

		if($IMUSRIP[0]['MAXARQCODE'] < 10000) {
			$valll = explode(substr($f_value[0]['IMUSRIP'],-4),$vall[1]);
			$apprno = strtoupper($vall[0].$srno.' / '.$currentdate1.' / '.substr($srno, -4).' / '.$currenttime1);
		} else {
			$valll = explode(substr($f_value[0]['IMUSRIP'],-5),$vall[1]);
			$apprno = strtoupper($vall[0].$srno.' / '.$currentdate1.' / '.substr($srno, -5).' / '.$currenttime1);
		}
		/////// Check Approval no-----------------
		
		//  INSERT INTO APPROVAL_REQUEST Table
		$ivqry = delete_test_dbquery("INSERT INTO APPROVAL_REQUEST select '".$ARQCODE[0]['MAXARQCODE']."', ARQYEAR, ARQSRNO, ATYCODE, ATMCODE, APMCODE, ATCCODE, APPRFOR, REQSTTO, APPRSUB, APPRDET, APPRSFR, APPRSTO, APPATTN, '".$sectionrow['APRAMNT']."', '".$sectionrow['APRAMNT']."', '".$sectionrow['APRAMNT']."', '".$sectionrow['BRNCODE']."', DEPCODE, TARNUMB, TARBALN, TARDESC, REQSTBY, RQBYDES, REQDESC, REQESEC, REQDESN, REQESEN, REQSTFR, RQFRDES, RQFRDSC, RQFRESC, RQFRDSN, RQFRESN, RQESTTO, RQTODES, RQTODSC, RQTOESC, RQTODSN, RQTOESN, '".$apprno."', APPSTAT, APPFRWD, APPINTP, INTPEMP, INTPDES, INTPDSC, INTPESC, INTPDSN, INTPESN, INTPAPR, INTSUGG, INTPFRD, INTPTOD, ADDUSER, ADDDATE, EDTUSER, EDTDATE, 'Y', DELUSER, DELDATE, APRCODE, APRHURS, APRDAYS, APRDUED, APPRMRK, APRTITL, FINSTAT, FINUSER, FINCMNT, FINDATE, TARVLCY, TARVLLY, EXPNAME, TARPRCY, TARPRLY, USRSYIP, PRJPRCS, PLANVAL, IMDUEDT, IMUSRCD, IMSTATS, IMFINDT, '".$srno."', TYPMODE, SUBCORE, BUDTYPE, BUDCODE, IMFNIMG, NXLVLUS, PRICODE, SUPCODE, SUPNAME, SUPCONT, PRODWIS, RESPUSR, ALTRUSR, '".$txt_approval_number."', ORGRECV, ORGRVUS, ORGRVDT, ORGRVDC, CNVRMOD, PURHEAD, APPTYPE, ADVAMNT, AGNSAPR, WRKINUSR, '".$srno."', BDPLANR, DYNSUBJ, TXTSUBJ, RMQUOTS, RMBDAPR, RMCLRPT, RMARTWK, RMCONAR, WARQUAR, CRCLSTK, PAYPERC, FNTARDT, RPTUSER, ACKUSER, ACKSTAT, ACKDATE, AGEXPDT, AGADVAM, DYSBFDT, DYSBTDT, APPICON, RTFNUSR, RMPONUM, AUTFRWD, EXPSRNO, QUOYEAR from APPROVAL_REQUEST where APRNUMB = '".$txt_approval_number."'");
		echo "<br>INSERT INTO APPROVAL_REQUEST select '".$ARQCODE[0]['MAXARQCODE']."', ARQYEAR, ARQSRNO, ATYCODE, ATMCODE, APMCODE, ATCCODE, APPRFOR, REQSTTO, APPRSUB, APPRDET, APPRSFR, APPRSTO, APPATTN, '".$sectionrow['APRAMNT']."', '".$sectionrow['APRAMNT']."', '".$sectionrow['APRAMNT']."', '".$sectionrow['BRNCODE']."', DEPCODE, TARNUMB, TARBALN, TARDESC, REQSTBY, RQBYDES, REQDESC, REQESEC, REQDESN, REQESEN, REQSTFR, RQFRDES, RQFRDSC, RQFRESC, RQFRDSN, RQFRESN, RQESTTO, RQTODES, RQTODSC, RQTOESC, RQTODSN, RQTOESN, '".$apprno."', APPSTAT, APPFRWD, APPINTP, INTPEMP, INTPDES, INTPDSC, INTPESC, INTPDSN, INTPESN, INTPAPR, INTSUGG, INTPFRD, INTPTOD, ADDUSER, ADDDATE, EDTUSER, EDTDATE, 'Y', DELUSER, DELDATE, APRCODE, APRHURS, APRDAYS, APRDUED, APPRMRK, APRTITL, FINSTAT, FINUSER, FINCMNT, FINDATE, TARVLCY, TARVLLY, EXPNAME, TARPRCY, TARPRLY, USRSYIP, PRJPRCS, PLANVAL, IMDUEDT, IMUSRCD, IMSTATS, IMFINDT, '".$srno."', TYPMODE, SUBCORE, BUDTYPE, BUDCODE, IMFNIMG, NXLVLUS, PRICODE, SUPCODE, SUPNAME, SUPCONT, PRODWIS, RESPUSR, ALTRUSR, '".$txt_approval_number."', ORGRECV, ORGRVUS, ORGRVDT, ORGRVDC, CNVRMOD, PURHEAD, APPTYPE, ADVAMNT, AGNSAPR, WRKINUSR, '".$srno."', BDPLANR, DYNSUBJ, TXTSUBJ, RMQUOTS, RMBDAPR, RMCLRPT, RMARTWK, RMCONAR, WARQUAR, CRCLSTK, PAYPERC, FNTARDT, RPTUSER, ACKUSER, ACKSTAT, ACKDATE, AGEXPDT, AGADVAM, DYSBFDT, DYSBTDT, APPICON, RTFNUSR, RMPONUM, AUTFRWD, EXPSRNO, QUOYEAR from APPROVAL_REQUEST where APRNUMB = '".$txt_approval_number."'";
		...
		if($ai == 1) {
			echo "<br>INSERT INTO approval_budget_planner_temp select '".$apprno."' , '".$budget_planner_temp[0]['MXAPRSRNO']."', APRPRID, APRMNTH, '".$sectionrow['APRAMNT']."', APPMNTH, APPYEAR, TARNUMB, '".$RESVALU."', '".$EXTVALU."', BUDMODE, APRYEAR, ADDUSER, ADDDATE, EDTUSER, EDTDATE, DELETED, DELUSER, DELDATE, '".$sectionrow['BRNCODE']."', APPMODE, EXPSRNO, EXISTVL, USEDVAL, ACCVRFY, TMTARNO, ATYCODE, PHDCODE, PGRCODE, ESECODE, DEPCODE from approval_budget_planner_temp where APRNUMB = '".$txt_approval_number."'";
			$ivqry = delete_test_dbquery("INSERT INTO approval_budget_planner_temp select '".$apprno."' , '".$budget_planner_temp[0]['MXAPRSRNO']."', APRPRID, APRMNTH, '".$sectionrow['APRAMNT']."', APPMNTH, APPYEAR, TARNUMB, '".$RESVALU."', '".$EXTVALU."', BUDMODE, APRYEAR, ADDUSER, ADDDATE, EDTUSER, EDTDATE, DELETED, DELUSER, DELDATE, '".$sectionrow['BRNCODE']."', APPMODE, EXPSRNO, EXISTVL, USEDVAL, ACCVRFY, TMTARNO, ATYCODE, PHDCODE, PGRCODE, ESECODE, DEPCODE, PRJCODE from approval_budget_planner_temp where APRNUMB = '".$txt_approval_number."'");

			/*//** $ivqry = delete_test_dbquery("INSERT INTO approval_budget_planner select '".$apprno."' , '".$budget_planner_temp[0]['MXAPRSRNO']."', APRPRID, APRMNTH, '".$sectionrow['APRAMNT']."', APPMNTH, APPYEAR, TARNUMB, '".$RESVALU."', '".$EXTVALU."', BUDMODE, APRYEAR, ADDUSER, ADDDATE, EDTUSER, EDTDATE, DELETED, DELUSER, DELDATE, '".$sectionrow['BRNCODE']."', 'N', EXPSRNO, EXISTVL, USEDVAL, DEPCODE, PRJCODE from approval_budget_planner_temp where APRNUMB = '".$txt_approval_number."'");
			$ivqry = delete_test_dbquery("update approval_budget_planner set APPMODE = 'Y' where APRNUMB = '".$apprno."'"); */
			...
			foreach($sql_descode_approval_mdhierarchy as $sectionrow) {
				$amrsrno++;
				$ivqry = delete_test_dbquery("INSERT INTO approval_mdhierarchy select APMCODE, AMHSRNO, APPHEAD, APPDESG, APPDAYS, APPRIOR, APPTITL, VRFYREQ, '".$apprno."', PBDAPPR from approval_mdhierarchy where APRNUMB = '".$apprno."'");
			}
		}
	}
}

function generate_multiple_branch_finish_process($txt_approval_number, $slt_topcore) {
	$ai = 0;
	$sql_descode = select_query_json("SELECT * from approval_request where RELAPPR = '".$txt_approval_number."' order by APRNUMB", "Centra", "TEST");
	foreach($sql_descode as $sectionrow) { $ai++;
		$sql_current_approval = select_query_json("SELECT * from approval_request where APRNUMB = '".$sectionrow['APRNUMB']."' and deleted = 'Y'", "Centra", "TEST");
		//  INSERT INTO APPROVAL_REQUEST Table
		$ivqry = delete_test_dbquery("INSERT INTO APPROVAL_REQUEST select '".$sql_current_approval[0]['ARQCODE']."', ARQYEAR, ARQSRNO, ATYCODE, ATMCODE, APMCODE, ATCCODE, APPRFOR, REQSTTO, APPRSUB, APPRDET, APPRSFR, APPRSTO, APPATTN, '".$sql_current_approval[0]['APPFVAL']."', '".$sql_current_approval[0]['APPFVAL']."', '".$sql_current_approval[0]['APPFVAL']."', '".$sql_current_approval[0]['BRNCODE']."', DEPCODE, TARNUMB, TARBALN, TARDESC, REQSTBY, RQBYDES, REQDESC, REQESEC, REQDESN, REQESEN, REQSTFR, RQFRDES, RQFRDSC, RQFRESC, RQFRDSN, RQFRESN, RQESTTO, RQTODES, RQTODSC, RQTOESC, RQTODSN, RQTOESN, '".$sectionrow['APRNUMB']."', APPSTAT, APPFRWD, APPINTP, INTPEMP, INTPDES, INTPDSC, INTPESC, INTPDSN, INTPESN, INTPAPR, INTSUGG, INTPFRD, INTPTOD, ADDUSER, ADDDATE, EDTUSER, EDTDATE, 'Y', DELUSER, DELDATE, APRCODE, APRHURS, APRDAYS, APRDUED, APPRMRK, APRTITL, FINSTAT, FINUSER, FINCMNT, FINDATE, TARVLCY, TARVLLY, EXPNAME, TARPRCY, TARPRLY, USRSYIP, PRJPRCS, PLANVAL, IMDUEDT, IMUSRCD, IMSTATS, IMFINDT, '".$sql_current_approval[0]['IMUSRIP']."', TYPMODE, SUBCORE, BUDTYPE, BUDCODE, IMFNIMG, NXLVLUS, PRICODE, SUPCODE, SUPNAME, SUPCONT, PRODWIS, RESPUSR, ALTRUSR, '".$txt_approval_number."', ORGRECV, ORGRVUS, ORGRVDT, ORGRVDC, CNVRMOD, PURHEAD, APPTYPE, ADVAMNT, AGNSAPR, WRKINUSR, '".$sql_current_approval[0]['ARQPCOD']."', BDPLANR, DYNSUBJ, TXTSUBJ, RMQUOTS, RMBDAPR, RMCLRPT, RMARTWK, RMCONAR, WARQUAR, CRCLSTK, PAYPERC, FNTARDT, RPTUSER, ACKUSER, ACKSTAT, ACKDATE, AGEXPDT, AGADVAM, DYSBFDT, DYSBTDT, APPICON, RTFNUSR, RMPONUM, AUTFRWD, EXPSRNO, QUOYEAR from APPROVAL_REQUEST where APRNUMB = '".$txt_approval_number."'");

		echo "<br>INSERT INTO APPROVAL_REQUEST select '".$sql_current_approval[0]['ARQCODE']."', ARQYEAR, ARQSRNO, ATYCODE, ATMCODE, APMCODE, ATCCODE, APPRFOR, REQSTTO, APPRSUB, APPRDET, APPRSFR, APPRSTO, APPATTN, '".$sql_current_approval[0]['APPFVAL']."', '".$sql_current_approval[0]['APPFVAL']."', '".$sql_current_approval[0]['APPFVAL']."', '".$sql_current_approval[0]['BRNCODE']."', DEPCODE, TARNUMB, TARBALN, TARDESC, REQSTBY, RQBYDES, REQDESC, REQESEC, REQDESN, REQESEN, REQSTFR, RQFRDES, RQFRDSC, RQFRESC, RQFRDSN, RQFRESN, RQESTTO, RQTODES, RQTODSC, RQTOESC, RQTODSN, RQTOESN, '".$sectionrow['APRNUMB']."', APPSTAT, APPFRWD, APPINTP, INTPEMP, INTPDES, INTPDSC, INTPESC, INTPDSN, INTPESN, INTPAPR, INTSUGG, INTPFRD, INTPTOD, ADDUSER, ADDDATE, EDTUSER, EDTDATE, 'Y', DELUSER, DELDATE, APRCODE, APRHURS, APRDAYS, APRDUED, APPRMRK, APRTITL, FINSTAT, FINUSER, FINCMNT, FINDATE, TARVLCY, TARVLLY, EXPNAME, TARPRCY, TARPRLY, USRSYIP, PRJPRCS, PLANVAL, IMDUEDT, IMUSRCD, IMSTATS, IMFINDT, '".$sql_current_approval[0]['IMUSRIP']."', TYPMODE, SUBCORE, BUDTYPE, BUDCODE, IMFNIMG, NXLVLUS, PRICODE, SUPCODE, SUPNAME, SUPCONT, PRODWIS, RESPUSR, ALTRUSR, '".$txt_approval_number."', ORGRECV, ORGRVUS, ORGRVDT, ORGRVDC, CNVRMOD, PURHEAD, APPTYPE, ADVAMNT, AGNSAPR, WRKINUSR, '".$sql_current_approval[0]['ARQPCOD']."', BDPLANR, DYNSUBJ, TXTSUBJ, RMQUOTS, RMBDAPR, RMCLRPT, RMARTWK, RMCONAR, WARQUAR, CRCLSTK, PAYPERC, FNTARDT, RPTUSER, ACKUSER, ACKSTAT, ACKDATE, AGEXPDT, AGADVAM, DYSBFDT, DYSBTDT, APPICON, RTFNUSR, RMPONUM, AUTFRWD, EXPSRNO, QUOYEAR from APPROVAL_REQUEST where APRNUMB = '".$txt_approval_number."'";
		...
		if($ai == 1) {
			$ivqry = delete_test_dbquery("INSERT INTO approval_budget_planner select '".$apprno."' , '".$budget_planner_temp[0]['MXAPRSRNO']."', APRPRID, APRMNTH, '".$sectionrow['APRAMNT']."', APPMNTH, APPYEAR, TARNUMB, '".$RESVALU."', '".$EXTVALU."', BUDMODE, APRYEAR, ADDUSER, ADDDATE, EDTUSER, EDTDATE, DELETED, DELUSER, DELDATE, '".$sectionrow['BRNCODE']."', 'N', EXPSRNO, EXISTVL, USEDVAL, DEPCODE, PRJCODE from approval_budget_planner_temp where APRNUMB = '".$txt_approval_number."'");
			$ivqry = delete_test_dbquery("update approval_budget_planner_temp set deleted = 'Y' where APRNUMB = '".$apprno."'"); 
			$ivqry = delete_test_dbquery("update approval_budget_planner set APPMODE = 'Y' where APRNUMB = '".$apprno."'"); 
	...
	foreach($sql_descode as $sectionrow) {
		...
		if ($sectionrow['ATYCODE'] != '1') {
			...
			$BRNHDCD = select_query_json("Select nvl(Max(BRNHDCD),0)+1 maxarqcode From approval_branch_head", "Centra", "TCS"); // subject group
			 ...
			 echo "<br>";
			 echo "INSERT INTO approval_branch_head select '".$BRNHDCD[0]['MAXARQCODE']."', BRNCODE, EMPSRNO, EMPCODE, EMPNAME, '".$_SESSION['tcs_usrcode']."', sysdate , '', '', 'N', '' , '' , '".$sectionrow_new['BRNHDSR']."', '".$apm_depcode."', '".$apm_tarnumb."', '".$apm_value."', '".$apm_code."', HDORDNO from approval_subject_add where APRNUMB = '".$txt_approval_number."' and ATYCODE = '".$sectionrow['ATYCODE']."' and BRNHDSR = '".$sectionrow_new['BRNHDSR']."'";
			*/
...
// To Find the End Date (Based on 10 AM to 7 PM)
...
// Function for Delete the against approval nos PO No
function func_delete_against_approvalno($txt_approval_number, $txt_against_approval, $slt_budgetmode) {
	$currentdate = strtoupper(date('d-M-Y h:i:s A'));
	// To Delete the PO from budget_planner_summary
	$bps_table="budget_planner_summary";
    $bps_fld=array();
    $bps_fld['DELUSER'] = $_SESSION['tcs_usrcode'];
    $bps_fld['DELDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;    
    $bps_fld['DELETED'] = 'Y';
    $where_bps = "APPNUMB = '".$txt_against_approval."' and DELETED = 'N'";
    $insert_bps = update_dbquery($bps_fld, $bps_table, $where_bps);
    // To Delete the PO from budget_planner_summary

    // To Delete the PO from non_ord_summary
	$nos_table="non_ord_summary";
    $nos_fld=array();
    $nos_fld['DELUSER'] = $_SESSION['tcs_usrcode'];
    $nos_fld['DELDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;    
    $nos_fld['DELETED'] = 'Y';
    $where_nos = "APPNUMB = '".$txt_against_approval."' and DELETED = 'N'";
    $insert_nos = update_dbquery($nos_fld, $nos_table, $where_nos);
    // To Delete the PO from non_ord_summary

    // To Delete the PO from non_req_summary
	$nrs_table="non_req_summary";
    $nrs_fld=array();
    $nrs_fld['DELUSER'] = $_SESSION['tcs_usrcode'];
    $nrs_fld['DELDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;    
    $nrs_fld['DELETED'] = 'Y';
    $where_nrs = "APPNUMB = '".$txt_against_approval."' and DELETED = 'N'";
    $insert_nrs = update_dbquery($nrs_fld, $nrs_table, $where_nrs);
    // To Delete the PO from non_req_summary



    // To Delete the PO from approval_budget_planner_temp
	$nrs_table="approval_budget_planner_temp";
    $nrs_fld=array();
    $nrs_fld['DELUSER'] = $_SESSION['tcs_usrcode'];
    $nrs_fld['DELDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;    
    $nrs_fld['DELETED'] = 'Y';
    $where_nrs = "APRNUMB = '".$txt_against_approval."' and DELETED = 'N'";
    $insert_nrs = update_dbquery($nrs_fld, $nrs_table, $where_nrs);
    // To Delete the PO from approval_budget_planner_temp

    // To Delete the PO from approval_budget_planner
	$nrs_table="approval_budget_planner";
    $nrs_fld=array();
    $nrs_fld['DELUSER'] = $_SESSION['tcs_usrcode'];
    $nrs_fld['DELDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;    
    $nrs_fld['DELETED'] = 'Y';
    ...
    if($slt_budgetmode == 2) { // Self Cheque
	    // To Delete the PO from self_cheque_req_summary
		$srs_table="self_cheque_req_summary";
	    $srs_fld=array();
	    $srs_fld['DELUSER'] = $_SESSION['tcs_usrcode'];
	    $srs_fld['DELDATE'] = 'dd-Mon-yyyy HH:MI:SS AM~~'.$currentdate;    
	    $srs_fld['DELETED'] = 'Y';
	    $where_srs = "APPNUMB = '".$txt_against_approval."' and DELETED = 'N'";
	    $insert_srs = update_dbquery($srs_fld, $srs_table, $where_srs);
	    // To Delete the PO from self_cheque_req_summary
	}

    if($insert_bps == 1 and $insert_nos == 1 and $insert_nrs == 1) {
    	return 1;
    } else {
    	return 0;
    }
...
// Function for Group by Key value in PHP Array
function group_by($key, $data) {
    $cmnt_att_arqsrno = array();
	foreach ($data as $docs_element) {
	    $cmnt_att_arqsrno[$docs_element[$key]][] = $docs_element;
	}
    return $cmnt_att_arqsrno;
}
// Function for Group by Key value in PHP Array

// To Generate Link from # hashtag
function remarks_split($aprnumb, $remarks) {
    preg_match_all("/(#[0-9]+)/", $remarks, $matches);
    $cnt_sql_remarks = count($matches[0]);
    $expl_url = explode("#",$_SERVER['REQUEST_URI']);

    for ($cnti = 0; $cnti < $cnt_sql_remarks; $cnti++) { 
        $matches_id = str_replace('#', '', $matches[0][$cnti]);
        $matches_ids[] = str_replace('#', '', $matches[0][$cnti]);
        $sql_remarks = select_query_json("select APRDOCS from approval_request_docs 
                                                    where aprnumb = '".$aprnumb."' and apdcsrn = '".$matches_id."'", 'Centra', 'TCS');
        $data_update[] = "<a href='".$expl_url[0]."|".$sql_remarks[0]['APRDOCS']."' style='color: #ff0000; font-size:16px; font-weight:bold;' title='Refer : ".$matches_id."'><i class='fa fa-paperclip fa-5 fa-rotate-90'></i> ".$matches_id."</a>";
    }
    ...
    //** print_r( $matches_ids );
    // rint_r( $data_update );
    $url_tag = str_replace($matches[0], $data_update, $remarks);
    echo $final_url = str_replace("|", "#", $url_tag);
    /* $exp_remarks = explode("#", $remarks);
    $cnt_sql_remarks = count($exp_remarks);

    $sql_repl = '';
    for ($cnti = 0; $cnti <= $cnt_sql_remarks; $cnti++) { 
        if($exp_remarks[$cnti] != 0) {
            $sql_remarks = select_query_json("select APRDOCS from approval_request_docs where aprnumb = '".$aprnumb."' and apdcsrn = '".$exp_remarks[$cnti]."'", 'Centra', 'TCS');
            $data_update = "<a href='#".$sql_remarks[0]['APRDOCS']."' style='color: #ff0000; font-size:16px; font-weight:bold;' title='Refer : ".$exp_remarks[$cnti]."'><i class='fa fa-paperclip fa-5 fa-rotate-90'></i> ".$exp_remarks[$cnti]."</a>";
            // $sql_repl .= str_replace("#".$exp_remarks[$cnti], $data_update, $remarks);
            $sql_repl .= preg_replace('/(?<!\S)#([0-9a-zA-Z]+)/', $data_update, $remarks);
        }
    } */
    // echo "<br>".$sql_repl;
} 
...
// to generate csv from php array
